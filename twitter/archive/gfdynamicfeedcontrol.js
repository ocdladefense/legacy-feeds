...
//google.feeds.FeedControl.prototype.createHTML = function(){ window.alert('Hello World!') }
//window.alert(google.feeds.FeedControl);
...
GFdynamicFeedControl.prototype.setup_ = function(container) {
 ...
 // window.alert(google.createHtml);
  this.feedControl = new google.feeds.FeedControl();
  
  //window.alert( this.feedControl.createHtml.toString() );
  //override the 
  //this.feedControl.createHtml = function(){ return "<p>overriding this function...</p>" }
...
GFdynamicFeedControl.prototype.feedLoaded_ = function(index, result) {
  ...
  if (result.error) {
    ...
    // if they choose to do something more creative.
...
GFdynamicFeedControl.prototype.createListEntries_ = function(resultIndex, node) {
  ...
  for (var i = 0; i < entries.length; i++) {
   ...
   // window.alert(entries[i].publishedDate);
    ...
    var link = this.createLink_(entries[i].link,
                                ...
                                this.options.linkTarget);//@jbernal
    ...
    //alert(listEntry.innerHTML);//@jbernal
...
/**
 * Helper function to build the current entry DOM elements.
 * @private
 */
 
 
 GFdynamicFeedControl.prototype.parseCurrentEntry_ = function(feedEntry) {
	var tmp = null;
	tmp = document.createElement('div');//'<div class="gfg-entry" style="">Some entry here</div>';//@jbernal
	tmp.className='gf-result';
	tmp.appendChild(document.createElement('div'));
	tmp.firstChild.className='gf-title';
	tmp.firstChild.appendChild(document.createElement('a'));
	tmp.firstChild.firstChild.className='gf-title';
	tmp.firstChild.firstChild.href='#';
	tmp.firstChild.firstChild.appendChild(document.createTextNode(feedEntry.title));//@jbernal
	return tmp;
...
if(newFeatures) this.current = this.parseCurrentEntry_(this.entries[this.entryIndex]);
  this.current.style.top = '0px';
 //alert(this.current.innerHTML);
  this.current.innerHTML=this.current.innerHTML.replace(/Willamalane:/g,'');//@jbernal
 ...
 * @private
 */
GFdynamicFeedControl.prototype.createLink_ = function(href, text, datePublished, opt_target) {
  var link = document.createElement('a');
  link.href = href;
  //link.innerHTML = text;
  var tmp = text.replace(/Willamalane:/,'');//@jbernal
  link.innerHTML = dateCleanUp( tmp );
  if (opt_target) {
...
function dateCleanUp( text ) {
	re=/relativePublishedDate(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sept|Oct|Nov|Dec)\s(\d{2})/gim;
	function removeLeadingZeros( p1 ) {
		if(parseInt(p1)<10) {
			try{return p1.charAt(1)	;} catch(e){ return true; }
		}
		else return p1	;	
	}
	return text.replace(re,removeLeadingZeros);
